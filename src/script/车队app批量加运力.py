...
for n in range(40):
    ...
    addcar_info = common.auv_md_40_cmd_161(library=library, param=car_info_param)
    addcar_result = eval(addcar_info["pBody"])
    assert addcar_result == {"a": 0}
    # app 车辆列表查看该车
    app_carinfo_list_param = common.auv_param_md_40_cmd_164(a=0, b=enterprise)
    app_carinfo_list_info = common.auv_md_40_cmd_164(library=library, param=app_carinfo_list_param)
    ...
    add_gua_car_info = common.auv_md_40_cmd_165(library=library, param=gua_car_info_param)
    add_gua_car_result = eval(add_gua_car_info["pBody"])
    assert add_gua_car_result == {"a": 0}
    # app 挂车列表查看该挂车
    app_gua_carinfo_list_info_param = common.auv_param_md_40_cmd_169(a=enterprise, b=0)
    app_gua_carinfo_list_info = common.auv_md_40_cmd_169(library=library, param=app_gua_carinfo_list_info_param)
    ...
    app_add_driver_info = common.auv_md_40_cmd_170(library=library, param=driver_param)
    app_add_driver_result = eval(app_add_driver_info["pBody"])
    assert app_add_driver_result == {"a": 0}
    # app 司机列表查看该司机
    app_driverinfo_list_param = common.auv_param_md_40_cmd_174(a=enterprise, b=0)
    app_driverinfo_list_info = common.auv_md_40_cmd_174(library=library, param=app_driverinfo_list_param)
    ...
    app_add_driver_ya_info = common.auv_md_40_cmd_176(library=library, param=driver_ya_param)
    app_add_driver_ya_result = eval(app_add_driver_ya_info["pBody"])
    assert app_add_driver_ya_result == {"a": 0}
    # app 押运员列表查看该押运员
    app_driverinfo_list_ya_param = common.auv_param_md_40_cmd_184(a=enterprise, b=0)
    app_driverinfo_list_ya_info = common.auv_md_40_cmd_184(library=library, param=app_driverinfo_list_ya_param)
    ...
    assert count == 1
    # app 添加运力
    app_add_capacity_param = common.auv_param_md_40_cmd_180(a=car_id, b=gua_car_id, c=driver_id,
                                                            d=driver_ya_id, e=enterprise, f=driver_uid,
                                                            g=driver_ya_uid)
    app_add_capacity_info = common.auv_md_40_cmd_180(library=library, param=app_add_capacity_param)
    app_add_capacity_result = eval(app_add_capacity_info["pBody"])
    assert app_add_capacity_result == {"a": 0}
    # app 运力列表查看该运力
    app_capacitylist_param = common.auv_param_md_40_cmd_181(a=enterprise, b=0)
    app_capacitylist_info = common.auv_md_40_cmd_181(library=library, param=app_capacitylist_param)
